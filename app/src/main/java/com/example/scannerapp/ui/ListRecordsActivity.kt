...
class ListRecordsActivity : BaseActivity(R.layout.activity_list_records), CoroutineScope {
        ...
        if (scanResult == null) {
          ...
          activityScope.launch {
            ...
            if (batchExists) {
              ...
              checkForEarlierExpiryBatches(scanResult)
  ...
  // Function to check and warn for batches with earlier expiry dates
  private suspend fun checkForEarlierExpiryBatches(scannedBatchNumber: String) {
    val scannedBatch = withContext(Dispatchers.IO) {
      batchDetailsViewModel.getBatchDetailsLiveDataByBatchNumber(scannedBatchNumber)
    } ?: return // if null, just return

    val earlierBatches = withContext(Dispatchers.IO) {
      batchDetailsViewModel.getBatchesWithEarlierExpiryDates(scannedBatch.expiryDate)
    }
